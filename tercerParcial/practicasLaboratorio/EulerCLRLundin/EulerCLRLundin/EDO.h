...
class EDO {
	...
	public:
		...
		double Euler(double xi, double xf, double y, double i, System::String^ fn) {
			double k;
			double h = (xf - xi) / i;
			double f = fun(xi, y, fn);
		...
		double RK(float x0, float y0, float x, double n, System::String^ fn) {
			double h = (x - x0) / n;
			double k1, k2, k3, k4, k5;
			...
			for (int i = 1; i <= n; i++) {
				k1 = h * fun(x0, y, fn);
				k2 = h * fun(x0 + 0.5*h, y + 0.5*k1, fn);
				k3 = h * fun(x0 + 0.5*h, y + 0.5*k2, fn);
				k4 = h * fun(x0 + h, y + k3, fn);
		...
		double fun(double x, double y, System::String^ fn) {
