...
class Derivadas {
	...
	public:
		...
		std::string rectaTangenteDOS(void) {
			std::string ecuacion = "";
		...
		System::String^ rectaTangenteCLR(void) {
			System::String^ ecuacion = "";
			if (-fxD * x[0] + y[0] < 0) ecuacion = fxD + "x" + " - " + fabs(-fxD * x[0] + y[0]);
			else if (-fxD * x[0] + y[0] > 0)	ecuacion = fxD + "x" + " + " + fabs(-fxD * x[0] + y[0]);
			else if (-fxD * x[0] + y[0] == 0)	ecuacion = fxD + "x";
			return ecuacion;
		}
};