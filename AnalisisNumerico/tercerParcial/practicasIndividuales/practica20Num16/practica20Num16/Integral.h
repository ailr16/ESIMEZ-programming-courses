...
class Integral {
	...
	public:
		...
		double Simpson38(double x0, double xn, int n, int ind) {
			double h = (xn - x0) / (3*n);
			double x;
			double s = 0;
			double AT;
			...
			for (int i = 1; i < 3*n; i++) {
				x = x0 + i * h;
				if (i % 3 == 0)	s = s + 2 * f(x, ind);
				else   s = s + 3 * f(x, ind);
			}
			AT = ((3*h)/ 8)*s;
			return AT;
		}
		double Simpson38(double x0, double xn, int n, System::String^ fn) {
			double h = (xn - x0) / (3 * n);
			double x;
			double s = 0;
			double AT;
